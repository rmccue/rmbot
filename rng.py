import irc
import sys, time, imp, os, re, threading
...
class RNGBot(irc.Bot):
	def __init__(self, config):
		args = (config.nick, config.name, config.channels)
	...
	def bind_commands(self):
		...
		for name, func in self.variables.iteritems(): 
			...
			if not hasattr(func, 'event'): 
				func.event = 'PRIVMSG'
			elif isinstance(func.event, str): func.event = func.event.upper()
...
def connect(config):
	bot = RNGBot(config)
	bot.run(config.host, config.port)
...
def main():
	...
	if not hasattr(config, 'name'): 
		config.name = 'Random Number Generator Bot'
	...
	while True:
		...
		warning = 'Warning: Disconnected. Reconnecting in %s seconds...' % 20
