...
def f_weather(bot, input): 
   ...
   if input.sender == '#talis': 
      if args[0].startswith('.weather '): return

   icao_code = input.group(2)
   if not icao_code: 
      return bot.reply('Try .weather London, for example?')
   ...
   if (not len(icao_code) == 4) or \
      ...
      icao_code = code(bot, icao_code)
   else: icao_code = icao_code.upper()

   if not icao_code: 
      bot.reply('No ICAO code found, sorry')
      return
   ...
   if 'Not Found' in bytes: 
      bot.say(icao_code+': no such ICAO code, or no NOAA data')
      return
   ...
   if metar[0] == 'AUTO': 
      metar = metar[1:]
   if metar[0] == 'VCU': 
      bot.reply(icao_code + ': no data provided')
      return
   ...
   bot.say(format.encode('utf-8') % args)
